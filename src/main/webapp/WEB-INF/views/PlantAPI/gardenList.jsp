...
<%
	...
	if(operationName.length == operationNameMap.size()){
		...
		for (int i = 0; i < searchNmArr.length; i++) {
		    String val = request.getParameter(searchNmArr[i]);
		    if (val != null && !val.equals("")) {
		        if ("word".equals(searchNmArr[i]) || "sText".equals(searchNmArr[i])) {
		            val = URLEncoder.encode(val, "UTF-8");
		        }
		        parameter += "&" + searchNmArr[i] + "=" + val;
		    }
		}
		//서버와 통신
		URL apiUrl = new URL("http://api.nongsaro.go.kr/service"+parameter);
